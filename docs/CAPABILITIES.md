...
| Post-onboarding Edit profile | P1 | Implemented: photography-first `/profile/edit` with sectional nav, live public preview, `profile_completion` strength, capability-specific saves. Dating location is server-authoritative via `GET /profile` `location.configured` and `GET /api/v1/profile/location` when omitted; Place picker uses D8N Places (`GET /places`, `PUT /profile/place`); device GPS write remains an optional onboarding/guard path (`PUT /profile/location`). Browser localStorage is not location authority. | `GET/PATCH /api/v1/profile`, options, preferences, prompts `PUT`, photos upload/attach/delete, `GET /places`, `GET/PUT /profile/location`, `PUT /profile/place` | `src/features/profile/EditProfilePage.test.tsx`, `src/features/profile/edit/DatingPlacePicker.test.tsx`, `src/features/discovery/RequireLocation.test.tsx` | Needs staging host; photo reorder/primary and language proficiency are backend gaps |
...
| Product notifications | P1 | Inbox, unread count, mark read, mark-all-read, and navigation from dating-event payloads (`like_received`, `match_created`, `opener_received`, `message_received`, `welcome`). Destination mapping is centralized. Email/push delivery preferences are live (`product_email_enabled`, `push_enabled`); browser push registration is not claimed. | `GET/PATCH/POST /api/v1/notifications*`; `GET/PATCH /api/v1/notifications/preferences` | `src/features/notifications/NotificationsPage.test.tsx`, `src/features/notifications/notificationDestination.test.ts`, `src/lib/api/notifications.test.ts`, `src/features/profile/SettingsPage.test.tsx` | Needs staging two-member event proof; SMS/WhatsApp and per-event toggles are not backend-supported |
