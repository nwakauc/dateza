...
| Match-gated text chat | P1 | Premium responsive conversation list, history, cursor pagination, selection, profile context, safety actions, and bounded text send implemented. Composer attaches one photo or video via D8N Chat Media: upload intent, direct-to-R2 PUT, then send `{ body?, attachment_uploads: [{ signed_id, media_kind }] }`. Thread maps `view_url`/`download_url`/`poster_url` to inline play, save, and video poster; pending/processing polls the message list; failed is terminal. Sender remove uses `DELETE .../messages/{id}/attachments/{id}` (attachment stays as `deleted: true`). Message report is `POST /reports` `target_type: message`. Additive parsers keep text-only payloads working. No unread badges: D8N conversation payloads do not include unread/read fields | Conversation/message APIs; `POST /conversations/{id}/attachments/uploads`; send `attachment_uploads`; `DELETE /conversations/{id}/messages/{id}/attachments/{id}`; `POST /reports` `target_type: message`; `chat.message.media`; server rate limits and block enforcement; no authoritative unread/read receipts, typing, reactions, or realtime | `src/features/chats/ChatsPage.test.tsx`, `src/lib/api/social.test.ts`, `src/lib/api/chatMedia.test.ts`, `src/lib/api/safety.test.ts`; browser QA with controlled populated fixtures | Needs staging two-member proof for image/video send, Range playback, attachment delete, and report-after-delete evidence |
