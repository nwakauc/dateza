...
export default function LikesPage() {
  ...
  return <div className="shell-page">
    ...
    {!loading && !error && matches.length > 0 ? <div className="match-grid">{matches.map((match) => { const photo = match.profile.photos[0]; const name = match.profile.display_name || "DateZA member"; return <article className="match-card" key={match.id}><Link to={`/profile/${match.profile.id}`} state={{ from: "likes" }} className="match-card__photo">{photo ? <img src={photo.url} width="480" height="600" loading="lazy" alt={`${name}'s profile`} /> : <span>{name[0]?.toUpperCase()}</span>}</Link><div className="match-card__body"><div><Link to={`/profile/${match.profile.id}`} state={{ from: "likes" }}><h2>{name}{match.profile.age ? `, ${match.profile.age}` : ""}</h2></Link><p>You matched</p></div><button type="button" aria-label={`Message ${name}`} onClick={() => void message(match)} disabled={starting === match.id}><ChatIcon />{starting === match.id ? "Opening…" : "Message"}</button></div></article>; })}</div> : null}
  </div>;
}
